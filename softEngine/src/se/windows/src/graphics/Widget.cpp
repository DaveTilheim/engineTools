...
void Widget::onLeftHoldClick(std::function<void(Widget *self)> lambda, std::function<void(Widget *self)> passLambda)
{
	if(!this->onLeftClickLambda)
	{
		this->onLeftClickLambda = new std::function<void()>();
	...
	*this->onLeftClickLambda = [=]()
	{
		if(this->leftClickBool or this->contains(static_cast<sf::Vector2f>(util::getMousePosition())))
		{
			if(util::isButtonPressed(sf::Mouse::Left))
			{
				lambda(this);
...
void Widget::onRightHoldClick(std::function<void(Widget *self)> lambda, std::function<void(Widget *self)> passLambda)
{
	if(!this->onRightClickLambda)
	{
	...
	*this->onRightClickLambda = [=]()
	{
		if(this->rightClickBool or this->contains(static_cast<sf::Vector2f>(util::getMousePosition())))
		{
